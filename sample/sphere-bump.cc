...
#include "sphere-bump.frag.h"
#include "sphere-bump.vert.h"
...
Sphere::Vertex::Vertex(float azimuthal, float polar)
    : azimuthal(azimuthal), polar(polar)
{}

bool
Sphere::Render(float radius, glm::mat4 transform, uint32_t time)
{
  ...
  float theta = float(time % 3000) / 3000.f * 2 * M_PI;
  base_technique_.Uniform(0, "theta", glm::vec<1, float>(theta));
...
void
Sphere::Bind(std::unique_ptr<zukou::GlTexture> texture0,
    std::unique_ptr<zukou::GlTexture> texture1)
{
...
Sphere::Init()
{
  ...
  const char* fragment_shader_source =
      wire_ ? color_fragment_shader_source : sphere_bump_fragment_shader_source;
  ...
  if (!vertex_shader_.Init(GL_VERTEX_SHADER, sphere_bump_vertex_shader_source))
    return false;
  ...
  vertex_array_.VertexAttribPointer(
      0, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex), 0, &gl_vertex_buffer_);
  vertex_array_.Enable(1);
  vertex_array_.VertexAttribPointer(1, 1, GL_FLOAT, GL_FALSE, sizeof(Vertex),
      offsetof(Vertex, polar), &gl_vertex_buffer_);
  ...
  if (wire_) {
    ...
    if (texture0_) {
      base_technique_.Bind(0, "", texture0_.get(), GL_TEXTURE_2D, &sampler_);
    }

    if (texture1_) {
      base_technique_.Bind(1, "", texture1_.get(), GL_TEXTURE_2D, &sampler_);
    }
...
Sphere::ConstructVertices()
{
  for (int i = +resolution_; i >= -resolution_; i--) {
    float polar = M_PI_2 * (float)i / (float)resolution_;
    for (int j = 0; j <= resolution_ * 4; j++) {
      float u = (float)j / (4.f * (float)resolution_);
      float azimuthal = M_PI_2 * u * 4.f;

      vertices_.emplace_back(azimuthal, polar);
    }
  }
}
