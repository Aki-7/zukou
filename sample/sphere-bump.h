...
{
  struct Vertex {
    Vertex(float azimuthal, float polar);
    float azimuthal;
    float polar;
  };
 ...
 public:
  ...
  void Bind(std::unique_ptr<zukou::GlTexture> texture0,
      std::unique_ptr<zukou::GlTexture> texture1);

  bool Render(float radius, glm::mat4 transform, uint32_t time = 0);
 ...
 private:
  ...
  std::unique_ptr<zukou::GlTexture> texture0_;
  std::unique_ptr<zukou::GlTexture> texture1_;
